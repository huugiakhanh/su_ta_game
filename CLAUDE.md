...
- `config.js` — hằng số thuần (độ phân giải logic, kích thước, vật lý, asset path, bảng ánh xạ sprite 8-bit `HAZARD_SPRITES`/`ENEMY_SPRITES`/`PROJECTILE_SPRITES`/`PLAYER_ANIMATIONS`). Mọi module khác import từ đây, không định nghĩa lại.
...
- `assets.js` — cache `images` + các hàm `load*`/`loadAssets()` (ảnh nền/vật cản tĩnh/item + manifest và mọi strip 8-bit trong `SPRITE_8BIT_IN_GAME`). Không đụng DOM ngoài `Image()`/`fetch`.
...
- `main.js` — entry point (script `type="module"` trong template): `resetGame()`, vòng lặp `frame()`, wiring nút bấm, gọi `loadAssets()`/`fitCanvas()`; `?viewer=1` nạp `viewer.js` thay vì chạy màn.
- `viewer.js` — chế độ xem sprite (`/gameplay/levels/trung-trac?viewer=1`): chọn asset → animation trong manifest, xem ×1/×3/×4, lưới, baseline, pivot, đánh dấu `hit_frame`. Công cụ soát asset, không phải gameplay.
...
- **Map**: level dài 12 × 768px logic (world-space, định vị obstacle/book/enemy qua `worldX(chunk, localX)`), nền vẽ bằng **layer parallax chồng nhau** (`sky` = trời+vật xa, opaque, phủ cả 270px; `foreground` = làng/cây, PNG nền TRONG SUỐT; `ground` = dải cỏ/đất — xem `BACKDROP_LAYERS`) với `GROUND_Y` cố định = `GROUND_LAYER_TOP + GROUND_GRASS_OFFSET` = **248** (mép cỏ của `ground.png` vẽ thu còn 42px). Nền là ảnh cũ vẽ THU NHỎ theo k (bật smoothing riêng cho lớp nền + landmark; tắt trước khi vẽ sprite) — `TODO_MAP`: nền sẽ được vẽ lại theo brief môi trường. Set-piece cốt truyện đặt qua `LANDMARKS` theo toạ độ world-X. Asset cần tạo xem [docs/asset-brief-trung-trac.md](docs/asset-brief-trung-trac.md).
